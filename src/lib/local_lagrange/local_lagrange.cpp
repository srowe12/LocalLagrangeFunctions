...
arma::mat LocalLagrange::assembleInterpolationMatrix(std::vector<double> local_centers_x, std::vector<double> local_centers_y){
        //Initialize matrix to all zeros. 
	arma::mat interp_matrix(local_centers_x.size()+3,local_centers_x.size()+3,arma::fill::zeros); 
	double distx = 0;
	double disty = 0;
	double dist = 0;
	...
	for (size_t row = 0; row<num_centers; row++){
		...
		interp_matrix(num_centers+1,row) = interp_matrix(row,num_centers+1) = local_centers_x[row];
		interp_matrix(num_centers+2,row) = interp_matrix(row,num_centers+2) = local_centers_y[row];
		
	}
	//Attach P and P^T here...
	
	//Move semantics maybe?
	return interp_matrix;
} 

void LocalLagrange::buildCoefficients(std::vector<double> local_centers_x, std::vector<double> local_centers_y, unsigned int local_index){
	arma::mat interp_matrix = assembleInterpolationMatrix(local_centers_x,local_centers_y);
